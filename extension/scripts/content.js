...
function getAllVisibleChildrenText(node) {
...
function elementWithTotal () {
    ...
    totalWords.forEach((word) => {
        const totalElements = findElementsWithText(document.body, word, isCaseSensitive = true);
...
function getTotalPrice() {
    ...
    for(var i = 0; i < total.length; i++){
        ...
        if(totalPrice){
            return {totalPrice, element: element.attributes};
        }
    }
...
function isCheckoutPage(){
    ...
    const elementWithOrder = findElementsWithText(document.body, "Order", isCaseSensitive = true);
    const elementWithSummary = findElementsWithText(document.body, "Summary", isCaseSensitive = true);
    // check condition on these
    // if yes then give total amount
    let {totalPrice, element} = getTotalPrice();
    if(totalPrice && elementWithOrder.length > 0 && elementWithSummary.length > 0){
        chrome.runtime.sendMessage({ cmd: 'finalTotal', totalPrice: totalPrice, element: element }, function (response) {
